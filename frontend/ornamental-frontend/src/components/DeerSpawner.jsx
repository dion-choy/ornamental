...
const DeerSpawner = ({ deerCount = 0 }) => {
  ...
  // Load the deer model
  const deerModel = useLoader(GLTFLoader, "/reindeer.glb"); // Place deer.glb in /public/models
  ...
  const deerPositions = useMemo(() => {
    ...
    for (let i = 0; i < deerCount; i++) {
      ...
      const y = -0.15; // Keep deers on the ground plane

      // Calculate rotation to face the center
      const position = new THREE.Vector3(x, y, z);
      const direction = new THREE.Vector3()
        .subVectors(centralPoint, position)
        .normalize(); // Direction vector towards the center
