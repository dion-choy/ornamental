...
export async function getUsers(userids) {
  ...
  const client = await clientPromise;
  const db = client.db("Ornamental");
  let users = await db.collection("users").find({ _id: { $in: useridsList } })
    .toArray();
...
export async function addPlayer(playerid, roomcode) {
  ...
  const client = await clientPromise;
  const db = client.db("Ornamental");
  let user = await db.collection("users").updateOne({ _id: playerid }, {
    $set: { room: roomcode.toString() },
  });
  return await db
    .collection("rooms")
    .updateOne({ code: roomcode.toString() }, {
...
export async function startSecretSanta(roomCode) {
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    console.log(
      await db.collection("users").updateOne({ _id: users[i] }, {
...
export async function getReceiverFromSanta(authorid) {
  ...
  if (result) {
    const userPair = result.secret_santa.user_pair.find((
      pair,
    ) => (pair.author.equals(authorOid)));
