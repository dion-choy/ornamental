...
export const SpiralAnimation = (props) => {
  ...
  // Function to increment the count of deers ended
  function incrDeersEnded() {
    setDeersEnded((prevDeer) => (prevDeer += 1));
  }
  ...
  useEffect(() => {
    ...
    // Fetch users and set player names and distortions
  ...
  useEffect(() => {
    // Fade away effect when all deers have ended
    if (deersEnded > playerNames.length) {
      setFadeAway({ opacity: 0 });
    }
  }, [deersEnded]);

  // Generate the spiral path with parameters (1100, -0.07, 5, 50)
  const spiralPath = generateSpiralPath(1100, -0.07, 5, 50);

  // Distortion function to move each square slightly on its own path
        ...
        return (
          ...
          >
            ...
            >
              ...
              />
              <p className={spiralStyle["reindeer-name"] + " text-2xl mx-5"}>
                {name}
              </p>
            </motion.div>
          </motion.div>
            ...
            >
              {4 > animStage > 0
                ? (
                  <>
                    <motion.h1
                      className="block text-5xl mt-40 font-extrabold"
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      transition={{ duration: 1 }}
                    >
              ...
              {animStage == 2
                ? <SpiralAnimation users={users} onComplete={incAnimStage} />
                : null}
              ...
              {animStage > 2 && animStage < 4
                ? (
                  <>
                    <motion.h2
                      className="block mt-40 text-3xl font-extrabold"
                    ...
                    <motion.div
                      className="flex mx-auto justify-center flex-row mt-48"
                      whileHover={{
                        scale: 1.4,
                        transition: {
                          duration: 1,
