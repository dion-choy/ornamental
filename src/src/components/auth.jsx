...
export default function Auth(props) {
    ...
    async function signUp() {
        let room = EJSON.parse(await getRoom(id));
        let res = await checkPlayer(userN, password, props.code);
        if (res === false) {
            setExists(true);
            let res;
            if (room.list_of_users.length == 0) {
                res = EJSON.parse(await createUser(password, userN, true));
            } else {
                res = EJSON.parse(await createUser(password, userN));
            }
